    ...
    try {
        ...
        if(leads.length != 0){
            // for (let i = 0; i < leads.length; i++) {
                ...
                console.log('This  is user data ', n);
                lead = {
                    UserId: n._id,
                    name: n.name
                }
        ...
        if (members.length != 0) {
            ...
            for (let i = 0; i < members.length; i++) {
                ...
                let proj = await Project.findOne({ projectName: members[i]}, (err, doc)=>{
                    if(doc){
                        console.log('SXksaaskxbasxj',doc);
                ...
                membersArray.push(membr);
                console.log('This is members array ',membersArray);
                console.log('Thsi is x value ', members);
                ...
                if(req.body.ismap == true){
                    ...
                    // console.log('This is internal Select' , selection[j].isSelected);
                        if(selection[j].isSelected == false){
                            // console.log('Selected in Checkbox ',selection[i].usrId);
                            response = await User.findByIdAndUpdate({ _id: selection[j].usrId }, { $set: { "isactive": false } }) //.then(ress => {
                                temp2 = response;  
                                // console.log('This is internal Select ',response);

                        }else{
                            response = await User.findByIdAndUpdate({ _id: selection[j].usrId }, { $set: { "isactive": true } }) //.then(ress => {
                                temp2 = response; 
                        }
                    }
